...
router.post('/commande', async (req, res) => {
  try {
    ...
    const newSOSCommande = new SOSCommande({
      // Champs historiques déjà utilisés côté utilisateur
      userId,
      nomPote,
      numeroBat,
      numeroChambre: parseInt(numeroChambre, 10),
      listeId: liste._id,
      sosId: sos._id,
      horaire,
      jour,
      ...
      // Champs ajoutés pour compatibilité côté liste
      listeName: liste.name || liste.Nom || '',
      liste: liste.name || liste.Nom || '',

      serviceId: sos.serviceId,
      serviceName: sos.serviceName || sos.name || '',
      nom_SOS: sos.name || sos.serviceName || '',

      description: sos.Description || sos.description || '',
      imageUrl: sos.image || sos.imageUrl || '',
      pointValeur: sos.pointValeur || 0,

      pnom_commande: userId,
