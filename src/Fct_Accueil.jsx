  ...
  return (
    <div className="podium-slot" style={{ '--delay': `${animDelay}s`, '--medal-color': medal.color, '--shadow': medal.shadow }}>
      <div className="candidate-card">
        ...
        <p className="cand-score">{candidate.score} pts</p>
      </div>
    ...
    <>
      ...
      <nav>
        ...
        <button
          ...
          onClick={(e) => {
            e.preventDefault();
            window.location.href = '/login_bde';
          }}
        >
          BDE
        </button>
          ...
          <>
            <div className="podium-wrap">
              {podiumOrder.map((cand, i) => (
                <PodiumStage key={cand?.id ?? i} candidate={cand} medal={podiumMedals[i]} animDelay={0.1 + i * 0.15} />
              ))}
            </div>
            ...
            {rest.length > 0 && (
              <div className="rest-list">
                ...
                {rest.map((cand, i) => (
                  <div key={cand.id} className="rest-row" style={{ '--row-delay': `${0.6 + i * 0.08}s` }}>
                    ...
                    <span className="row-score">{cand.score} pts</span>
                  </div>
                ))}
              </div>
            )}
